...
from recombine import entropy_metrics, recombine_core
...
def run_validation() -> Dict[str, Any]:
    ...
    try:
        ...
        validation_context["test_scenarios"] = [
            scenario["name"] for scenario in test_scenarios
        ]
        ...
        validation_context["performance_metrics"] = aggregated_results[
            "performance_metrics"
        ]
        validation_context["quality_assessment"] = aggregated_results[
            "quality_assessment"
        ]
        ...
        validation_context["validation_summary"] = _generate_validation_summary(
            validation_context
        )
...
def validate_workflow_integration() -> Dict[str, Any]:
    ...
    try:
        ...
        successful_integrations = sum(
            1 for result in integration_tests.values() if result.get("success", False)
        )
    ...
    scenarios = [
        ...
        },
        {
            ...
            "input_data": {
                ...
                "semantic_relations": [
                    {
                        "source": "complex_entity_1",
                        "target": "complex_entity_2",
                        "relation": "contains",
                    }
                ],
                "processing_depth": 3,
        ...
        },
        {
            ...
            "input_data": {
                ...
                "intent_classification": {
                    "modal_analysis": 0.3,
                    "epistemic_processing": 0.25,
                    "normative_reasoning": 0.2,
                    "action_theory": 0.15,
...
def _run_scenario_validation(
    scenario: Dict[str, Any], validation_context: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            scenario_result["stage_results"]["recombine"] = {
                ...
                "quality_score": recombined_result.get("payload", {})
                .get("synthesis_metadata", {})
        ...
        try:
            ...
            scenario_result["stage_results"]["lambda"] = {
                ...
                "complexity_score": lambda_result.get("payload", {})
                .get("normalization_metadata", {})
                .get("complexity_score", 0.0),
        ...
        try:
            ...
            scenario_result["stage_results"]["translate"] = {
                ...
                "quality_score": translate_result.get("payload", {})
        ...
        try:
            ...
            scenario_result["stage_results"]["obdc"] = {
                ...
                "tokens_emitted": obdc_result.get("payload", {})
                .get("emission_metadata", {})
                .get("total_tokens", 0),
                "quality_score": obdc_result.get("payload", {})
        ...
        successful_stages = sum(
            1
            for stage in scenario_result["stage_results"].values()
            if stage.get("success", False)
        )
        total_stages = len(scenario_result["stage_results"])
        scenario_result["success"] = (
            successful_stages == total_stages and len(scenario_result["issues"]) == 0
        )

        scenario_result["end_time"] = time.time()
        scenario_result["total_execution_time"] = (
            scenario_result["end_time"] - scenario_result["start_time"]
        )
    ...
    try:
        ...
        for stage_name, stage_data in aggregated["stage_results"].items():
            if stage_data["total_count"] > 0:
                ...
                if (
                    stage_name in ["recombine", "translate"]
                    and "quality_sum" in stage_data
                ):
                    stage_data["avg_quality"] = (
                        stage_data["quality_sum"] / stage_data["total_count"]
                    )
                elif stage_name == "lambda" and "complexity_sum" in stage_data:
                    stage_data["avg_complexity"] = (
                        stage_data["complexity_sum"] / stage_data["total_count"]
                    )
                elif stage_name == "obdc" and "tokens_sum" in stage_data:
                    stage_data["avg_tokens"] = (
                        stage_data["tokens_sum"] / stage_data["total_count"]
                    )

        # Performance metrics
        if execution_times:
            aggregated["performance_metrics"]["avg_total_execution_time"] = sum(
                execution_times
            ) / len(execution_times)
        ...
        for stage_name, stage_data in aggregated["stage_results"].items():
            if stage_data["total_count"] > 0:
                ...
                aggregated["performance_metrics"]["stage_success_rates"][
                    stage_name
                ] = success_rate
        ...
        if quality_scores:
            aggregated["quality_assessment"]["overall_quality_score"] = sum(
                quality_scores
            ) / len(quality_scores)
...
def _test_registry_recombine_integration() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "integration_quality": (
                "good" if len(domains) > 0 and result.get("status") == "ok" else "poor"
            ),
        }
...
def _test_recombine_lambda_integration() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "output_format_valid": "normalized_structures"
            in lambda_result.get("payload", {}),
            "integration_quality": (
                "good" if lambda_result.get("status") == "ok" else "poor"
            ),
        }
...
def _test_lambda_translate_integration() -> Dict[str, Any]:
    ...
    try:
        ...
        mock_lambda = {
            ...
            "payload": {
                "normalized_structures": {
                    "test_structure": {
                        "lambda_expressions": [
                            {"type": "test", "lambda_expression": "λx.x"}
                        ],
                        "reduction_steps": [],
                    }
                },
        ...
        return {
            "success": translate_result.get("status") == "ok",
            "input_format_compatible": True,
            "output_format_valid": "unified_natural_language"
            in translate_result.get("payload", {}),
            "integration_quality": (
                "good" if translate_result.get("status") == "ok" else "poor"
            ),
        }
...
def _test_translate_obdc_integration() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "integration_quality": (
                "good" if obdc_result.get("status") == "ok" else "poor"
            ),
        }
...
def _test_data_flow_integrity() -> Dict[str, Any]:
    ...
    try:
        ...
        # Step 2: Recombine
        recombined = recombine_core.merge_outputs(
            test_input, domains[:3]
        ...
        return {
            ...
            "integration_quality": (
                ...
                else "good" if integrity_score >= 0.7 else "poor"
            ),
        }
    ...
    try:
        ...
        try:
            ...
            error_scenarios.append(
                {"scenario": "error_input_translate", "handled": True}
            )
        except Exception:
            error_scenarios.append(
                {"scenario": "error_input_translate", "handled": False}
            )
...
def _assess_overall_validation_success(validation_context: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        # Success criteria
        criteria = {
            "scenario_success_rate": performance_metrics.get("successful_scenarios", 0)
            / performance_metrics.get("total_scenarios", 1),
            "stage_success_rates": performance_metrics.get("stage_success_rates", {}),
            "performance_acceptable": performance_metrics.get(
                "avg_total_execution_time", float("inf")
            )
            < 10.0,  # Under 10 seconds
            "quality_acceptable": validation_context.get("quality_assessment", {}).get(
                "overall_quality_score", 0.0
            )
            > 0.5,
        }
        ...
        stage_success = all(
            rate >= 0.8 for rate in criteria["stage_success_rates"].values()
        )

        overall_success = (
            scenario_success
            and stage_success
            and criteria["performance_acceptable"]
            and criteria["quality_acceptable"]
        )
...
def _generate_validation_summary(validation_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        summary = {
            "validation_status": (
                "PASS" if validation_context.get("overall_success", False) else "FAIL"
            ),
            ...
            "overall_success_rate": performance_metrics.get("successful_scenarios", 0)
            / max(performance_metrics.get("total_scenarios", 1), 1),
            "avg_execution_time": performance_metrics.get(
                "avg_total_execution_time", 0.0
            ),
    ...
    try:
        ...
        if avg_time > 5.0:
            recommendations.append(
                "Consider optimizing pipeline performance - average execution time exceeds 5 seconds"
            )
        ...
        for stage, rate in stage_success_rates.items():
            if rate < 0.9:
                recommendations.append(
                    f"Improve {stage} stage reliability - success rate is {rate:.1%}"
                )

        # Quality recommendations
        quality_score = validation_context.get("quality_assessment", {}).get(
            "overall_quality_score", 0.0
        )
        if quality_score < 0.7:
            recommendations.append(
                "Enhance processing quality - overall quality score is below acceptable threshold"
            )

        if not recommendations:
            recommendations.append(
                "Pipeline validation successful - no critical issues identified"
            )

    except Exception:
        recommendations.append(
            "Unable to generate specific recommendations due to validation analysis errors"
        )
