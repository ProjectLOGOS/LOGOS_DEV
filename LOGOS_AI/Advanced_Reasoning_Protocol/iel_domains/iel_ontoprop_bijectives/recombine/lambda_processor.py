    ...
    def substitute(
        ...
        if self.expr_type == LambdaType.VARIABLE:
            ...
            if self.bound_var == var:
                ...
                renamed_body = self.body.substitute(
                    self.bound_var, LambdaExpression(LambdaType.VARIABLE, fresh_var)
                )
                substituted_body = renamed_body.substitute(var, replacement)
                return LambdaExpression(
                    LambdaType.ABSTRACTION, None, fresh_var, substituted_body
                )
            else:
                # Safe to substitute in body
                new_body = self.body.substitute(var, replacement)
                return LambdaExpression(
                    LambdaType.ABSTRACTION, None, self.bound_var, new_body
                )
...
class Combinators:
    ...
    def S() -> LambdaExpression:
        ...
        # Build xz(yz)
        result_app = LambdaExpression(
            LambdaType.APPLICATION, None, function=xz_app, argument=yz_app
        )
...
class TypeInferencer:
    ...
    def infer_type(
        self, expr: LambdaExpression, context: Optional[Dict[str, LambdaType_]] = None
    ) -> LambdaType_:
        ...
        if expr.expr_type == LambdaType.VARIABLE:
            ...
            return LambdaType_(TypeCategory.FUNCTION, [var_type, body_type])
            ...
            expected_func_type = LambdaType_(
                TypeCategory.FUNCTION, [arg_type, result_type]
            )
...
class LambdaCalculusEngine:
    ...
    def parse_expression(self, expr_str: str) -> LambdaExpression:
        ...
        if expr_str.startswith("λ") or expr_str.startswith("\\"):
            ...
            if dot_pos > 0:
                ...
                return LambdaExpression(
                    LambdaType.ABSTRACTION, None, var_part, body_expr
                )
    ...
    def reduce_expression(
        self, expr: LambdaExpression, strategy: Optional[ReductionStrategy] = None
    ) -> ReductionResult:
        """
        ...
        try:
            ...
            while reduction_count < self.max_reduction_steps:
                ...
                reduction_count += 1
                steps.append(
                    f"Step {reduction_count}: {step_description} → {current_expr}"
                )
            ...
            self.logger.debug(
                f"Reduction completed: {reduction_count} steps, normal form: {is_normal}"
            )
            return result
    ...
    def _single_reduction_step(
        self, expr: LambdaExpression, strategy: ReductionStrategy
    ...
    def _normal_order_reduction(
        ...
        if expr.expr_type == LambdaType.APPLICATION:
            ...
            if (
                expr.function.expr_type == LambdaType.ABSTRACTION
                or expr.function.expr_type == LambdaType.COMBINATOR
            ):

                # Perform beta reduction
                result = expr.function.body.substitute(
                    expr.function.bound_var, expr.argument
                )
                return (
                    result,
                    f"β-reduction: {expr.function.bound_var} := {expr.argument}",
                )
            ...
            if expr.body:
                ...
                if reduced_body is not None:
                    new_expr = LambdaExpression(
                        LambdaType.ABSTRACTION, None, expr.bound_var, reduced_body
                    )
    ...
    def _applicative_order_reduction(
        self, expr: LambdaExpression
    ) -> Tuple[Optional[LambdaExpression], str]:
    ...
    def _is_normal_form(self, expr: LambdaExpression) -> bool:
        ...
        if expr.expr_type == LambdaType.APPLICATION:
            ...
            return self._is_normal_form(expr.function) and self._is_normal_form(
                expr.argument
            )
    ...
    def evaluate_with_context(
        self, expr: LambdaExpression, context: Dict[str, Any]
    ) -> ReductionResult:
        ...
        for var_name, value in context.items():
            if isinstance(value, tuple) and len(value) == 3:
                ...
                expr_with_context = expr_with_context.substitute(var_name, trinity_func)
            elif isinstance(value, (int, float, bool)):
                # Constant value
                const_expr = LambdaExpression(LambdaType.CONSTANT, value)
                expr_with_context = expr_with_context.substitute(var_name, const_expr)
