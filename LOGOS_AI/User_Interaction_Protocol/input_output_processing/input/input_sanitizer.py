...
from system_utillities.shared.message_formats import UIPRequest
...
class ContentValidator(BaseValidator):
    ...
    def validate(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "issues": issues,
            "metadata": {
                "blocked_terms_found": len(
                    [t for t in self.blocked_terms if t in content_lower]
                )
            },
        }
...
class InputSanitizer:
    ...
    def _initialize_validators(self):
        ...
        self.logger.info(
            f"Input sanitizer initialized with {len(self.validators)} validators"
        )
    ...
    def sanitize_input(self, request: UIPRequest) -> Dict[str, Any]:
        ...
        if result["valid"]:
            ...
            self.logger.warning(
                f"Input validation failed: {len(all_issues)} issues found"
            )
