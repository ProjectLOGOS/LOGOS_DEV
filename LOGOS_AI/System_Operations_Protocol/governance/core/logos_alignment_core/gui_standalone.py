...
class LOGOSGUI:
    def __init__(self, nexus):
        ...
        tk.Frame(self.root, bg="#00bfff", width=2).pack(
        ...
        self.scrollbar = tk.Scrollbar(
            self.chat_frame, orient=tk.VERTICAL, command=self.chat_log.yview
        )
        ...
        self.input_entry = tk.Entry(
            self.input_frame, bg="#333333", fg="#ffffff", font=("Helvetica", 12)
        )
        self.input_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.send_btn = ttk.Button(
            self.input_frame, text="Send", command=self.handle_send, style="TButton"
        )
        ...
        self.knot_canvas = tk.Canvas(
            self.right_frame, width=150, height=150, bg="#1a1a1a", highlightthickness=0
        )
        ...
        # Welcome message
        self.append_to_chat(
            "LOGOS: Welcome! I'm running with 11/13 libraries loaded. Ask me anything!"
        )
            ...
            try:
                if self.animation_state == "listening":
                    ...
                    for _ in range(10):  # 1Hz pulse
                        self.draw_trinity_knot(
                            color if _ % 2 else self.lighten_color(color, 0.2)
                        )
                        time.sleep(0.1)
                    ...
                    for _ in range(10):
                        self.draw_trinity_knot(
                            color if _ % 2 else self.lighten_color(color, 0.1)
                        )
                        time.sleep(0.1)
                else:  # stasis
                    colors = [
    ...
    def update_voice_mode(self):
        ...
        if self.voice_mode_var.get() == "Voice Activated" and not hasattr(
            self, "voice_thread"
        ):
            self.append_to_chat(
                "System: Voice Activated mode enabled (requires microphone)"
            )
