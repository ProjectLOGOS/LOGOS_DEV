...
class KripkeModel:
    ...
    def accessible_worlds(self, from_world: str) -> List[str]:
        ...
        return [
            to_world for to_world in self.worlds.keys()
    ...
    def set_accessibility(self, from_world: str, to_world: str, accessible: bool) -> None:
...
class ModalLogicEngine:
    ...
    def validate_modal_formula(
        ...
        try:
            ...
            countermodel_worlds = [
                ...
                if w not in satisfaction_worlds
            ]
            ...
            self.logger.debug(f"Modal validation completed: {is_valid} (confidence: {confidence:.3f})")
            return result
    ...
    def _evaluate_formula(
        ...
        if formula.operator is None:
            ...
            trace.append(f"Necessity check in {world}: {result} (checked {len(accessible_worlds)} worlds)")
            ...
            result = any_true
            trace.append(f"Possibility check in {world}: {result} (checked {len(accessible_worlds)} worlds)")
            ...
            operator_name = "Knowledge" if formula.operator == ModalOperator.KNOWLEDGE else "Belief"
            ...
            result = True
            satisfaction_worlds = [world]
            trace.append(f"Default handling for {formula.operator} in {world}: {result}")
    ...
    def _calculate_modal_confidence(
        ...
        # Consistency bonus
        consistency_bonus = 0.1 if len(countermodel_worlds) == 0 or len(satisfaction_worlds) == 0 else 0
    ...
    def _analyze_modal_properties(
        ...
        properties = {
            ...
            'satisfaction_degree': satisfied_count / total_worlds if total_worlds > 0 else 0,
            'is_tautology': satisfied_count == total_worlds,
            'is_contradiction': satisfied_count == 0,
            'is_contingent': 0 < satisfied_count < total_worlds,
            'accessibility_dependencies': self._analyze_accessibility_deps(model, satisfaction_worlds),
            'world_coverage': {
                'satisfied': satisfaction_worlds,
                'total_worlds': total_worlds,
                'coverage_percentage': (satisfied_count / total_worlds * 100) if total_worlds > 0 else 0
            }
        }
        ...
        if formula.operator:
            properties['modal_system_properties'] = {
                'requires_reflexivity': formula.operator in [ModalOperator.KNOWLEDGE, ModalOperator.NECESSITY],
    ...
    def _analyze_accessibility_deps(self, model: KripkeModel, satisfaction_worlds: List[str]) -> Dict[str, Any]:
        ...
        return {
            'relevant_accessibility_ratio': relevant_relations / total_relations if total_relations > 0 else 0,
            'satisfied_world_connectivity': len(satisfaction_worlds) > 1,
            'isolated_satisfied_worlds': [
                w for w in satisfaction_worlds 
                if not any(model.is_accessible(w, other) for other in satisfaction_worlds if other != w)
            ]
        }
    ...
    def validate_modal_consistency(self, formulas: List[Union[str, ModalFormula]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'consistency_score': sum(r.confidence for r in results) / len(results) if results else 0
        }
    ...
    def _check_mutual_consistency(self, results: List[ModalValidationResult]) -> bool:
        ...
        for result in results:
            if result.is_valid:
                common_worlds = common_worlds.intersection(set(result.satisfaction_worlds))
