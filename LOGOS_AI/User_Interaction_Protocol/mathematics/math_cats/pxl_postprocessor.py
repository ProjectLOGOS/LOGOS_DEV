...
try:
    from .pxl_schema import (
        PXLRelation, PXLAnalysisResult, ConsistencyReport, ConsistencyViolation,
        TrinityVector, ModalProperties, PXLRelationType, PXLConsistencyLevel,
        ValidationResult, PXLAnalysisConfig
    )
except ImportError:
...
class ProcessedOutput:
    ...
    def get_content_summary(self) -> str:
        ...
        return f"{self.format_type.value.upper()} output ({len(self.content)} characters)"
...
class RelationNetworkVisualizer:
    ...
    def _create_empty_network_message(self) -> str:
        ...
        ax.text(0.5, 0.5, 'No relations meet minimum strength threshold', 
...
class TrinityHeatmapVisualizer:
    ...
    def create_trinity_heatmap(self, trinity_vectors: Dict[str, TrinityVector]) -> str:
        ...
        try:
            ...
            # Prepare data
            concepts = list(trinity_vectors.keys())[:20]  # Limit to prevent overcrowding
            ...
            for i, concept in enumerate(concepts):
                ...
                matrix[i] = [trinity_vec.essence, trinity_vec.generation, trinity_vec.temporal]
    ...
    def _create_empty_heatmap_message(self) -> str:
        ...
        fig, ax = plt.subplots(figsize=self.config.figure_size)
        ax.text(0.5, 0.5, 'No Trinity vectors available', 
                ha='center', va='center', fontsize=14, transform=ax.transAxes)
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.axis('off')
...
class ConsistencyDashboardVisualizer:
    ...
    def create_consistency_dashboard(self, consistency_report: ConsistencyReport) -> str:
        ...
        try:
            ...
            if consistency_report.violations_by_type:
                ...
                violation_counts = [len(violations) for violations in consistency_report.violations_by_type.values()]
                
                ax1.pie(violation_counts, labels=violation_types, autopct='%1.1f%%', startangle=90)
                ax1.set_title('Violations by Type')
            else:
                ax1.text(0.5, 0.5, 'No violations found', ha='center', va='center', transform=ax1.transAxes)
                ax1.set_title('Violations by Type')
            ...
            for level in [PXLConsistencyLevel.CRITICAL_VIOLATION, 
            ...
            if consistency_report.local_consistency_scores:
                concepts = list(consistency_report.local_consistency_scores.keys())[:10]  # Top 10
                scores = [consistency_report.local_consistency_scores[concept] for concept in concepts]
                
                ax3.barh(concepts, scores, color='skyblue')
                ax3.set_title('Local Consistency Scores')
                ax3.set_xlabel('Consistency Score')
            else:
                ax3.text(0.5, 0.5, 'No local scores available', ha='center', va='center', transform=ax3.transAxes)
                ax3.set_title('Local Consistency Scores')
            ...
            ax4.arrow(0, 0, np.cos(score_angle), np.sin(score_angle), 
                     head_width=0.05, head_length=0.1, fc=color, ec=color, linewidth=3)
            
            ax4.set_xlim(-1.2, 1.2)
            ax4.set_ylim(0, 1.2)
            ax4.set_aspect('equal')
            ax4.axis('off')
            ax4.text(0, -0.2, f'Global Score: {global_score:.2f}', 
...
class OutputFormatter:
    ...
    def format_to_json(self, analysis_result: PXLAnalysisResult) -> str:
        ...
        data = {
            ...
            'trinity_analysis': {
                concept: asdict(vector) for concept, vector in analysis_result.trinity_analysis.items()
            },
            'modal_analysis': {
                concept: asdict(props) for concept, props in analysis_result.modal_analysis.items()
            },
        ...
        if self.config.pretty_print:
            return json.dumps(data, indent=2, default=serialize_object, ensure_ascii=False)
        else:
    ...
    def format_to_xml(self, analysis_result: PXLAnalysisResult) -> str:
        ...
        ET.SubElement(metadata_elem, 'processing_time').text = str(analysis_result.processing_time)
        ...
        ET.SubElement(consistency_elem, 'total_violations').text = str(analysis_result.consistency_report.total_violations)
        ET.SubElement(consistency_elem, 'global_consistency_score').text = str(analysis_result.consistency_report.global_consistency_score)
    ...
    def format_to_csv(self, analysis_result: PXLAnalysisResult) -> str:
        ...
        writer.writerow([
            ...
            'strength', 'confidence', 'trinity_coherence', 'modal_necessity'
        ])
    ...
    def format_to_markdown(self, analysis_result: PXLAnalysisResult) -> str:
        ...
        md_content.append(f"**Processing Time:** {analysis_result.processing_time:.2f} seconds")
        md_content.append("")
        ...
        md_content.append(f"- **Total Violations:** {analysis_result.consistency_report.total_violations}")
        md_content.append("")
        ...
        if analysis_result.consistency_report.total_violations > 0:
            ...
            if analysis_result.consistency_report.resolution_recommendations:
                ...
                md_content.append("")
                for i, recommendation in enumerate(analysis_result.consistency_report.resolution_recommendations, 1):
        ...
        if analysis_result.trinity_analysis:
            ...
            for concept, vector in list(analysis_result.trinity_analysis.items())[:10]:  # Top 10
                md_content.append(f"| {concept} | {vector.essence:.3f} | {vector.generation:.3f} | {vector.temporal:.3f} |")
            md_content.append("")
        ...
        for rel_type, relations in relations_by_type.items():
            md_content.append(f"### {rel_type.replace('_', ' ').title()} ({len(relations)} relations)")
            md_content.append("")
            
            # Show top relations
            sorted_relations = sorted(relations, key=lambda r: r.strength, reverse=True)
            for relation in sorted_relations[:5]:  # Top 5
                md_content.append(f"- {relation.source_concept} → {relation.target_concept} (strength: {relation.strength:.2f})")
...
class ReportTemplateEngine:
    ...
    def __init__(self, config: PostprocessingConfig):
        ...
        if self.config.custom_template_path and Path(self.config.custom_template_path).exists():
            loader = FileSystemLoader(str(Path(self.config.custom_template_path).parent))
    ...
    def render_template(
        self, 
        analysis_result: PXLAnalysisResult,
...
class PXLPostprocessor:
    ...
    def process_analysis_result(self, analysis_result: PXLAnalysisResult) -> ProcessedOutput:
        """
        ...
        try:
            ...
            if self.config.output_format == OutputFormat.JSON:
                ...
                content = self.template_engine.render_template(analysis_result, "default")
            else:
                # Fallback to JSON
                content = self.formatter.format_to_json(analysis_result)
    ...
    def _generate_visualizations(self, analysis_result: PXLAnalysisResult) -> Dict[str, Any]:
        ...
        try:
            if VisualizationType.RELATION_NETWORK in self.config.visualization_types:
                visualizations['relation_network'] = self.network_visualizer.create_network_visualization(
            ...
            if VisualizationType.TRINITY_HEATMAP in self.config.visualization_types:
                if analysis_result.trinity_analysis:
                    visualizations['trinity_heatmap'] = self.trinity_visualizer.create_trinity_heatmap(
                        analysis_result.trinity_analysis
                    )
            
            if VisualizationType.CONSISTENCY_DASHBOARD in self.config.visualization_types:
                visualizations['consistency_dashboard'] = self.consistency_visualizer.create_consistency_dashboard(
