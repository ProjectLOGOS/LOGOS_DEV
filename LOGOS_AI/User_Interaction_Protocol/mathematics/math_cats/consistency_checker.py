...
class LogicalConsistencyAnalyzer:
    ...
    def _detect_direct_contradictions(self, relations: List[Any]) -> List[ConsistencyViolation]:
        ...
        for relation in relations:
            if hasattr(relation, 'relation_type') and hasattr(relation, 'source_concept'):
                ...
                elif 'not_' in relation.relation_type.value or 'contradiction' in relation.relation_type.value:
    ...
    def _check_inference_consistency(self, relations: List[Any]) -> List[ConsistencyViolation]:
    ...
    def _detect_definitional_circularity(self, relations: List[Any]) -> List[ConsistencyViolation]:
        ...
        for relation in relations:
            if (hasattr(relation, 'relation_type') and 
                relation.relation_type.value == 'definitional_relation'):
    ...
    def _check_property_consistency(self, concept_properties: Dict[str, Dict[str, Any]]) -> List[ConsistencyViolation]:
        ...
        for concept, properties in concept_properties.items():
            ...
            if boolean_props.get('temporal', False) and boolean_props.get('eternal', False):
                violation = ConsistencyViolation(
    ...
    def _build_inference_graph(self, relations: List[Any]) -> nx.DiGraph:
        ...
        for relation in relations:
            if hasattr(relation, 'relation_type') and hasattr(relation, 'source_concept'):
                if relation.relation_type.value in ['logical_implication', 'entails', 'causal_relation']:
                    graph.add_edge(
                        relation.source_concept,
                        relation.target_concept,
                        relation=relation
...
class TrinityConsistencyAnalyzer:
    ...
    def _check_vector_validity(self, trinity_vectors: Dict[str, Tuple[float, float, float]]) -> List[ConsistencyViolation]:
    ...
    def _check_relational_coherence(
        ...
        for relation in relations:
            if (hasattr(relation, 'source_concept') and hasattr(relation, 'target_concept') and
                relation.source_concept in trinity_vectors and relation.target_concept in trinity_vectors):
                ...
                if hasattr(relation, 'strength'):
                    ...
                    if abs(coherence - expected_coherence) > 0.4:  # Significant mismatch
                        violation = ConsistencyViolation(
                            violation_type=ConsistencyViolationType.TRINITY_VECTOR_INCOHERENCE,
                            severity=ViolationSeverity.MODERATE,
                            scope=ConsistencyScope.LOCAL,
                            description=f"Trinity coherence mismatch for relation {relation.source_concept}→{relation.target_concept}",
                            involved_concepts=[relation.source_concept, relation.target_concept],
                            evidence={
    ...
    def _check_theological_constraints(self, trinity_vectors: Dict[str, Tuple[float, float, float]]) -> List[ConsistencyViolation]:
        """Check Trinity-specific theological constraints"""
        violations = []
        
        # Check for divine concepts
        divine_concepts = [concept for concept in trinity_vectors.keys() 
...
class ModalConsistencyAnalyzer:
    ...
    def check_modal_consistency(
        self,
        modal_statements: Dict[str, Dict[str, Any]],
        relations: List[Any]
    ) -> List[ConsistencyViolation]:
    ...
    def _check_modal_axioms(self, modal_statements: Dict[str, Dict[str, Any]]) -> List[ConsistencyViolation]:
        ...
        for concept, modal_props in modal_statements.items():
            ...
            if modal_props.get('necessary', False) and modal_props.get('impossible', False):
                violation = ConsistencyViolation(
    ...
    def _check_necessity_consistency(self, modal_statements: Dict[str, Dict[str, Any]]) -> List[ConsistencyViolation]:
        """Check necessity/possibility consistency"""
        violations = []
        
        for concept, modal_props in modal_statements.items():
            # If something is necessary, it must also be possible
            if modal_props.get('necessary', False) and not modal_props.get('possible', True):
                violation = ConsistencyViolation(
                    violation_type=ConsistencyViolationType.MODAL_INCONSISTENCY,
                    severity=ViolationSeverity.MAJOR,
                    scope=ConsistencyScope.LOCAL,
    ...
    def _check_accessibility_consistency(self, relations: List[Any]) -> List[ConsistencyViolation]:
        """Check accessibility relation consistency"""
        violations = []
        
        # This is a simplified check - full modal logic would require more sophisticated analysis
        modal_relations = [r for r in relations 
...
class PXLConsistencyChecker:
    ...
    def check_consistency(
        ...
        try:
            # Generate cache key
            cache_key = self._generate_cache_key(relations, trinity_vectors, concept_properties, modal_statements)
            ...
            violations_by_severity = self._organize_violations_by_severity(all_violations)
            
            # Identify critical violations
            critical_violations = violations_by_severity.get(ViolationSeverity.CRITICAL, [])
            
            # Calculate consistency scores
            global_score = self._calculate_global_consistency_score(all_violations, len(relations))
            ...
            analysis_metadata = {
                ...
                'concepts_with_trinity_vectors': len(trinity_vectors) if trinity_vectors else 0,
                'concepts_with_properties': len(concept_properties) if concept_properties else 0,
                'modal_statements_analyzed': len(modal_statements) if modal_statements else 0,
    ...
    def _calculate_local_consistency_scores(
        ...
        for concept, concept_viols in concept_violations.items():
            penalty = sum(self.severity_weights[v.severity] * v.confidence for v in concept_viols)
    ...
    def _calculate_cluster_consistency_scores(
        ...
        for cluster, cluster_viols in cluster_violations.items():
            penalty = sum(self.severity_weights[v.severity] * v.confidence for v in cluster_viols)
    ...
    def _generate_resolution_recommendations(
        ...
        if len(violations) > 10:
            recommendations.append("Consider systematic review of concept definitions and relations")
        
        if any(v.violation_type == ConsistencyViolationType.TRINITY_VECTOR_INCOHERENCE for v in violations):
            recommendations.append("Review Trinity vector assignments for theological accuracy")
        
        if any(v.violation_type == ConsistencyViolationType.MODAL_INCONSISTENCY for v in violations):
            recommendations.append("Check modal logic statements for consistency with axioms")
    ...
    def _calculate_consistency_metrics(
        ...
        return {
            ...
            'critical_violation_rate': len([v for v in violations if v.severity == ViolationSeverity.CRITICAL]) / max(total_violations, 1),
            'avg_violation_confidence': sum(v.confidence for v in violations) / max(total_violations, 1),
            'logical_violation_ratio': len([v for v in violations if v.violation_type == ConsistencyViolationType.LOGICAL_CONTRADICTION]) / max(total_violations, 1),
            'trinity_violation_ratio': len([v for v in violations if v.violation_type == ConsistencyViolationType.TRINITY_VECTOR_INCOHERENCE]) / max(total_violations, 1),
            'modal_violation_ratio': len([v for v in violations if v.violation_type == ConsistencyViolationType.MODAL_INCONSISTENCY]) / max(total_violations, 1)
        }
    ...
    def _generate_cache_key(
        ...
        key_data = {
            ...
            'trinity_concepts': sorted(trinity_vectors.keys()) if trinity_vectors else [],
            'property_concepts': sorted(concept_properties.keys()) if concept_properties else [],
            'modal_concepts': sorted(modal_statements.keys()) if modal_statements else []
        }
