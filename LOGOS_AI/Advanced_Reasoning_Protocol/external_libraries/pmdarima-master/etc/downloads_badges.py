...
def millify(n):
    ...
    final_output = (
        one_decimal
        if n > 1e6 and not one_decimal.is_integer()
        else int(round(final_num, 0))
    )
...
pyramid_arima = json.loads(
    session.get("https://api.pepy.tech/api/projects/pyramid-arima").text
)
...
# Millify the totals
total_downloads = millify(
    pyramid_arima["total_downloads"] + pmdarima["total_downloads"]
)
