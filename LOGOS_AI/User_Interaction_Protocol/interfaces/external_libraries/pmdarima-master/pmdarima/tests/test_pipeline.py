...
class TestIllegal:
    ...
    def test_non_unique_names(self):
        ...
        with pytest.raises(ValueError) as ve:
            Pipeline(
                [("stage", BoxCoxEndogTransformer()), ("stage", ARIMA(order=(0, 0, 0)))]
            )
    ...
    def test_names_in_params(self):
        ...
        with pytest.raises(ValueError) as ve:
            Pipeline(
                [("steps", BoxCoxEndogTransformer()), ("stage", ARIMA(order=(0, 0, 0)))]
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Pipeline(
            [("fourier", FourierFeaturizer(m=12)), ("arma", ARIMA(order=(2, 0, 0)))]
        ),
        Pipeline(
            [("fourier", FourierFeaturizer(m=12)), ("arima", ARIMA(order=(2, 1, 0)))]
        ),
        Pipeline(
            [("log", LogEndogTransformer(lmbda=10)), ("arima", ARIMA(order=(2, 1, 0)))]
        ),
...
def test_pipeline_predict_inverse_transform(
    pipeline, X, inverse_transform, return_conf_ints
):
    X_train, X_test = X
