...
class AuditLogger:
    ...
    def log_event(
        ...
        component: str = "adaptive_inference",
    ) -> bool:
        """
        Log an audit event.
    ...
    def _write_structured_event(self, event: AuditEvent):
        ...
        try:
            ...
            if (
                ...
                and self.log_path.stat().st_size > self.max_file_size
            ):
...
def log_event(
    event_type: str, payload: Optional[Union[Dict[str, Any], str]] = None
) -> bool:
    """
    ...
    try:
        ...
        # Fallback logging
        print(
            f"Audit event logging failed: {event_type}, {payload}, {e}",
...
def get_audit_status() -> Dict[str, Any]:
    ...
    try:
        ...
        status = {
            ...
            "log_file_size": (
                audit_logger.log_path.stat().st_size
        ...
        # Count backup files
        backup_files = list(
            audit_logger.log_path.parent.glob(f"{audit_logger.log_path.stem}.*")
        )
        status["backup_files_count"] = len(
            [f for f in backup_files if f.suffix.lstrip(".").isdigit()]
        )
