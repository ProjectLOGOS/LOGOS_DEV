...
def test_load_and_encode(model_name: str) -> None:
    ...
    assert isinstance(
        embeddings, Tensor
    ), "Embeddings should be a tensor for sparse encoders"
    assert len(embeddings) == len(
        sentences
    ), "Number of embeddings should match number of sentences"

    decoded_embeddings = model.decode(embeddings)
    assert len(decoded_embeddings) == len(
        sentences
    ), "Decoded embeddings should match number of sentences"
    assert all(
        isinstance(emb, list) for emb in decoded_embeddings
    ...
    assert (
        len(single_sentence_emb) == 1
    ), "Single sentence embedding dict should not be empty"

    # If we're using a string instead of a list, we should get a single tensor embedding
    single_sentence_emb_tensor = model.encode(
        "A single sentence.", convert_to_tensor=False
    )
    assert isinstance(
        single_sentence_emb_tensor, Tensor
    ), "Encoding a single sentence with convert_to_tensor=False should return a tensor"
    assert (
        single_sentence_emb_tensor.dim() == 1
    ), "Single sentence embedding tensor should be 1D"
