...
# === EMBEDDED GPT ENGINE ===
# GPTLOGOSEngine class embedded to eliminate external dependency
...
When users ask questions that would benefit from these tools, respond with JSON in this format:
{"action": "use_tool", "tool": "tetragnos|telos|thonoc", "data": {...}}

For regular conversation, just respond naturally as the LOGOS AI assistant."""

    async def handle_conversation(self, message: str, session_id: str) -> str:
        """Handle a conversation turn with GPT"""
        ...
        try:
            ...
            messages.extend(self.conversation_history[session_id][-10:])  # Last 10 messages
            ...
            try:
                ...
                if tool_data.get("action") == "use_tool":
                    return await self.execute_tool(tool_data, session_id)
    ...
    async def handle_text_analysis_gpt(self, data: dict, session_id: str) -> str:
        """Handle TETRAGNOS text analysis via GPT"""
        return "TETRAGNOS text analysis integration pending"
    
    async def handle_forecasting_gpt(self, data: dict, session_id: str) -> str:
        """Handle TELOS forecasting via GPT"""
        return "TELOS forecasting integration pending"
    
    async def handle_theorem_proving_gpt(self, data: dict, session_id: str) -> str:
        """Handle THONOC theorem proving via GPT"""
        return "THONOC theorem proving integration pending"
