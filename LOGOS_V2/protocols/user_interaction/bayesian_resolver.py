...
class TrinityPriorDatabase:
    ...
    def update_prior(self, term: str, evidence: Dict[str, float], learning_rate: float = 0.1):
...
class BayesianTrinityInferencer:
    ...
    def infer_trinity_vector(
        ...
        weights = weights if weights and len(weights) == len(keywords) else [1.0] * len(keywords)
        ...
        posterior_uncertainty = self._calculate_posterior_uncertainty([e_final, g_final, t_final])
        ...
        self.logger.debug(f"Bayesian inference completed: T=({e_final:.3f}, {g_final:.3f}, {t_final:.3f}), confidence={confidence:.3f}")
        ...
        for term in ambiguous_terms:
            try:
                ...
                result = self.infer_trinity_vector(
                    combined_keywords,
                    weights=[2.0] + [0.5] * (len(combined_keywords) - 1),  # Boost target term
    ...
    def _calculate_confidence(
        ...
        coverage = len(evidence_terms) / len(original_weights) if original_weights else 0
    ...
    def _calculate_dimension_variance(self, evidence_list: List[float], mean_val: float) -> float:
