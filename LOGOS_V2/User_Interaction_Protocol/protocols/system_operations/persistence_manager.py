...
def persist_adaptive_state(profile: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if manager.config.adaptive_path.exists():
            ...
            logger.info(f"Adaptive state persisted: {file_size} bytes to {manager.config.adaptive_path}")
            return True
        else:
...
def load_adaptive_state() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if "persistence_meta" in profile and "checksum" in profile["persistence_meta"]:
            ...
            profile_for_validation = {k: v for k, v in profile.items() if k != "persistence_meta"}
            ...
            if expected_checksum != actual_checksum:
                logger.warning(f"Adaptive state checksum mismatch: expected {expected_checksum}, got {actual_checksum}")
...
def _calculate_profile_checksum(profile: Dict[str, Any]) -> str:
    ...
    try:
        ...
        profile_json = json.dumps(profile, sort_keys=True, ensure_ascii=True, default=str)
...
def _create_backup(file_path: Path, max_backups: int = 5):
    ...
    try:
        ...
        _cleanup_old_backups(file_path.parent, file_path.stem, file_path.suffix, max_backups)
...
def _cleanup_old_backups(backup_dir: Path, file_stem: str, file_suffix: str, max_backups: int):
...
def get_persistence_status() -> Dict[str, Any]:
    ...
    try:
        ...
        status = {
            ...
            "adaptive_state_size": manager.config.adaptive_path.stat().st_size if manager.config.adaptive_path.exists() else 0,
            "config": {
        ...
        if manager.config.adaptive_path.exists():
            ...
            backup_count = len(list(manager.config.adaptive_path.parent.glob(backup_pattern)))
