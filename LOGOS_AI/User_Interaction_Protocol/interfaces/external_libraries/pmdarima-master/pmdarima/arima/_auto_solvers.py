...
class _StepwiseFitWrapper(_SolverMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_k = (
            100 if self.exec_context.max_steps is None else self.exec_context.max_steps
        )
    ...
    def solve(self):
        ...
        while self.start_k < self.k < self.max_k:
            ...
            if (
                ...
                and self._do_fit((p, d, q), (P - 1, D, Q, m))
            ):
                P -= 1
                continue
            ...
            if (
                Q > 0
                and self.k < self.max_k
                and self._do_fit((p, d, q), (P, D, Q - 1, m))
            ):
                Q -= 1
                continue
            ...
            if (
                p > 0
                and self.k < self.max_k
                and self._do_fit((p - 1, d, q), (P, D, Q, m))
            ):
                p -= 1
                continue
            ...
            if (
                q > 0
                and self.k < self.max_k
                and self._do_fit((p, d, q - 1), (P, D, Q, m))
            ):
                q -= 1
                continue
        ...
        if (
            self.exec_context.max_steps is not None
...
def _sort_and_filter_fits(models):
    ...
    filtered = [
        (mod, ic) for mod, _, ic in models if mod is not None and np.isfinite(ic)
    ]
