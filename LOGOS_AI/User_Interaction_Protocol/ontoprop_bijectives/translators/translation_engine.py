...
def convert_to_nl(normalized_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for structure_type, structure_data in lambda_structures.items():
            nl_output = _translate_structure(structure_type, structure_data, translation_context)
            translation_context["natural_language_outputs"][structure_type] = nl_output
            
            # Generate semantic mappings
            semantic_mapping = _generate_semantic_mapping(structure_type, structure_data, nl_output)
        ...
        quality_metrics = _calculate_translation_quality(translation_context, unified_nl)
        translation_context["translation_quality"] = quality_metrics
        
        logger.info(f"NL translation completed: quality={quality_metrics.get('overall_quality', 0.0):.3f}")
        
        return {
            "status": "ok",
            "payload": {
                "unified_natural_language": unified_nl,
                "structured_translations": translation_context["natural_language_outputs"],
...
def _generate_semantic_mapping(structure_type: str, structure_data: Dict[str, Any], nl_output: str) -> Dict[str, Any]:
    ...
    return {
        "formal_elements": list(structure_data.keys()) if isinstance(structure_data, dict) else [],
...
def _apply_grammar_rules(semantic_mapping: Dict[str, Any]) -> List[str]:
    ...
    return semantic_mapping.get("natural_elements", ["structured", "conceptual", "representation"])
...
def _determine_translation_mode(lambda_structures: Dict[str, Any]) -> str:
    ...
    try:
        ...
        has_formal_logic = any("logic" in key.lower() for key in lambda_structures.keys())
        has_natural_lang = any("natural" in key.lower() for key in lambda_structures.keys())
        has_combinators = any("combinator" in key.lower() for key in lambda_structures.keys())
...
def _translate_structure(structure_type: str, structure_data: Dict[str, Any], 
    ...
    try:
        ...
        if "combinator" in structure_type.lower():
            nl_text = _translate_combinator_structure(lambda_expressions, reduction_steps)
            ...
            nl_text = _translate_standard_structure(lambda_expressions, semantic_content)
        ...
        return {
            ...
            "translation_confidence": _calculate_translation_confidence(structure_data, nl_text)
        }
...
def _translate_combinator_structure(expressions: List[Dict[str, Any]], 
                                  reduction_steps: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        for expr in expressions:
            ...
            if combinator_name in ["S", "K", "I"]:
                ...
                desc = descriptions.get(combinator_name, "performs combinator operation")
                combinator_descriptions.append(f"The {combinator_name} combinator {desc}")
            else:
                combinator_descriptions.append(f"A {expr_type} expression performs logical transformation")
...
def _translate_logical_structure(expressions: List[Dict[str, Any]], 
                               semantic_content: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if entities:
            ...
            if len(entity_names) == 1:
                ...
                parts.append(f"involving {', '.join(entity_names[:-1])} and {entity_names[-1]}")
        ...
        if intentions:
            intent_descriptions = [intent.get("description", "intends") for intent in intentions]
...
def _translate_standard_structure(expressions: List[Dict[str, Any]], 
                                semantic_content: Dict[str, Any]) -> str:
...
def _generate_structural_description(structure_data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if reduction_count > 0:
            parts.append(f"{reduction_count} reduction step{'s' if reduction_count > 1 else ''}")
...
def _synthesize_unified_representation(translation_context: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if not translations:
            ...
            return "This represents a complex expression that " + " Additionally, it ".join(
                [t.rstrip('.') for t in translations]
            ) + "."
...
def _calculate_translation_confidence(structure_data: Dict[str, Any], nl_text: str) -> float:
    ...
    try:
        ...
        text_quality = 1.0 if nl_text and not nl_text.startswith("[Translation unavailable") else 0.0
        text_length = min(len(nl_text.split()) / 20.0, 1.0)  # Normalize to reasonable length
...
def _calculate_translation_quality(translation_context: Dict[str, Any], 
                                 unified_nl: str) -> Dict[str, float]:
    ...
    try:
        ...
        successful_translations = sum(
            1 for output in nl_outputs.values()
            if not output.get("natural_language_text", "").startswith("[Translation unavailable")
        )
