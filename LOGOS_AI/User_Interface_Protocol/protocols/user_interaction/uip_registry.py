...
class UIPRegistry:
    ...
    def create_context(
        self, session_id: str, user_input: str, metadata: Dict[str, Any] = None
    ) -> UIPContext:
        ...
        try:
            ...
            for dep_step in handler.dependencies:
                if dep_step not in context.step_results:
                    raise ValueError(
                        f"Dependency {dep_step.value} not satisfied for {step.value}"
                    )

            # Execute handler with timeout
            result = await asyncio.wait_for(
                handler.handler_func(context), timeout=handler.timeout_seconds
            )
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "handler_status": {step.value: step in self.handlers for step in UIPStep},
        }
...
def register_uip_handler(
    step: UIPStep,
    dependencies: List[UIPStep] = None,
    timeout: int = 30,
    critical: bool = True,
):
