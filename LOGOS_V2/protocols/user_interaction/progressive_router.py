...
from protocols.shared.message_formats import UIPRequest, UIPResponse, MessageValidator
from protocols.user_interaction.uip_registry import uip_registry, UIPContext, UIPStep, UIPStatus, register_uip_handler
...
class ProgressiveRouter:
    ...
    def _create_response_from_context(self, context: UIPContext, start_time: float) -> UIPResponse:
        ...
        return UIPResponse(
            ...
            metadata={
                ...
                'step_breakdown': {
                    step.value: step in context.step_results 
                    for step in UIPStep
                }
            },
    ...
    def _extract_response_text(self, context: UIPContext) -> str:
        ...
        if UIPStep.STEP_6_RESPONSE_SYNTHESIS in context.step_results:
            ...
            if isinstance(synthesis_result, dict) and 'response_text' in synthesis_result:
        ...
        if UIPStep.STEP_8_EGRESS_DELIVERY in context.step_results:
            ...
            if isinstance(egress_result, dict) and 'formatted_response' in egress_result:
    ...
    def _extract_confidence_score(self, context: UIPContext) -> float:
        ...
        if UIPStep.STEP_1_LINGUISTIC in context.step_results:
            ...
            if isinstance(linguistic_result, dict) and 'confidence' in linguistic_result:
    ...
    def _extract_ontological_vector(self, context: UIPContext) -> Optional[Dict[str, float]]:
    ...
    def get_active_sessions(self) -> Dict[str, Dict]:
        ...
        return {
            session_id: {
                ...
                'last_activity': max(ctx.get('timestamp', 0) for ctx in contexts.values()) if contexts else 0
            }
